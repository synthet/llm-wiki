...
description: Use Gemini CLI through the subagent-orchestrator MCP server for review-only external CLI feedback in LLM Wiki.
...
- Use the `subagent-orchestrator` MCP server (project key **`llm-wiki-subagent-orchestrator`**).
