...
# /wiki-lint — Validate canonical product knowledge

Run `llmwiki validate --json` to verify source digests, locator bounds/quotes, reviewed evidence,
dependencies, contradictions, and generated-page conflicts. This complements (and does not replace)
the framework docs checks below.
...
0. **Run product validation first:** `llmwiki validate --json`.
1. **Run framework documentation validation** (from repo root):
...
2. **Scan all INDEX.md files** — build a list of every page referenced in indexes.
3. **Scan all .md files in `docs/`** — build a list of every page that exists on disk and note which living docs lack OKF frontmatter or a non-empty `type`.
3. **Compare** to find:
